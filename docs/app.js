...
const state = { payload: {}, view: "all", queryTab: "", filters: { query: "", status: "", sentiment: "", domain: "" }, sortKey: "current_rank", sortDirection: "asc", page: 1, pageSize: 50, screenshotIndex: 0, screenshotZoom: 1 };
...
function bindAuth() {
  if (sessionStorage.getItem(AUTH_KEY) === "true") return showDashboard();
  document.querySelector("#loginForm").addEventListener("submit", async (event) => {
    ...
    document.querySelector("#loginError").textContent = "Invalid login or password.";
  });
}
function sourceRows() {
  ...
  return state.payload.views?.[state.view] || [];
}
function filteredRows() {
  return sourceRows().filter((item) => (!state.filters.query || item.query === state.filters.query)
    && (!state.filters.status || item.status === state.filters.status)
    && (!state.filters.sentiment || item.sentiment === state.filters.sentiment)
    && (!state.filters.domain || [item.domain_entity, item.parent_domain, item.domain].includes(state.filters.domain)));
}
function sortedRows(rows) {
  return [...rows].sort((left, right) => {
    let leftValue = left[state.sortKey];
    let rightValue = right[state.sortKey];
    if (["first_seen", "last_seen"].includes(state.sortKey)) {
...
function compactList(values) {
  return values?.length ? values.slice(0, 6).map((value) => `<span>${escapeHtml(value)}</span>`).join("") : "-";
}
...
function renderQueryHealth() {
  const entries = Object.entries(state.payload.query_health || {});
  document.querySelector("#queryHealth").innerHTML = entries.length ? entries.map(([query, item]) => `
    <article class="health-card"><span>${escapeHtml(query)}</span><strong>${escapeHtml(item.score)}/100</strong><em class="trend ${escapeHtml(item.trend)}">${escapeHtml(item.trend)}</em></article>
  `).join("") : '<p class="empty">Query health scores will appear after snapshot export.</p>';
}
function renderTable() {
  ...
  body.innerHTML = pageRows.length ? pageRows.map((item) => `
    <tr>
      <td class="query-cell">${escapeHtml(item.query)}</td><td>${escapeHtml(item.current_rank || "-")}</td><td>${escapeHtml(item.previous_rank || "-")}</td><td>${item.rank_delta ?? "-"}</td><td>${badge(item.status)}</td>
      <td class="title-cell">${escapeHtml(item.title)}</td><td class="url-cell"><button class="url-history" data-url="${escapeHtml(item.url)}" type="button">${escapeHtml(item.url)}</button></td>
      <td>${escapeHtml(item.domain)}</td><td>${escapeHtml(item.domain_entity || item.parent_domain || "-")}</td><td>${tagBadges(item.domain_tags)}</td><td>${badge(item.sentiment || "neutral")}</td><td>${badge(item.risk_level || "none")}</td>
      <td>${escapeHtml(item.risk_keywords || "-")}</td><td>${formatDate(item.first_seen)}</td><td>${formatDate(item.last_seen)}</td><td>${item.date_published ? escapeHtml(item.date_published) : "unknown"}</td><td>${badge(item.source_type || "organic")}</td>
    </tr>`).join("") : '<tr><td colspan="17" class="empty">No rows match this view</td></tr>';
...
function fillSelect(selector, values, label) {
  document.querySelector(selector).innerHTML = `<option value="">${label}</option>` + values.map((value) => `<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`).join("");
}
...
function renderQueryTabs(queries) {
  ...
  root.innerHTML = '<button class="query-tab active" data-query="" type="button">All current top-10</button>' + queries.map((query) => `<button class="query-tab" data-query="${escapeHtml(query)}" type="button">${escapeHtml(query)}</button>`).join("");
  root.querySelectorAll(".query-tab").forEach((button) => button.addEventListener("click", () => {
...
function renderDomains() {
  const rows = state.payload.domains || [];
  document.querySelector("#domainCount").textContent = `${rows.length} entities`;
  domains.innerHTML = rows.length ? rows.map((domain) => `<article class="domain-card"><h3>${escapeHtml(domain.domain)}</h3><p>${escapeHtml((domain.raw_domains || []).join(", "))}</p><div>${tagBadges(domain.tags)}</div><div class="domain-stats"><span>Total</span><strong>${domain.total}</strong><span>Best rank</span><strong>${domain.best_rank || "-"}</strong><span>Risky</span><strong>${domain.risky || 0}</strong><span>Negative</span><strong>${domain.negative || 0}</strong><span>Positive</span><strong>${domain.positive || 0}</strong><span>Neutral</span><strong>${domain.neutral || 0}</strong></div></article>`).join("") : '<p class="empty">Domain summary will appear after the first export.</p>';
}
function renderGallery() {
  const shots = state.payload.screenshots || [];
  document.querySelector("#screenshotCount").textContent = `${shots.length} captures`;
  screenshots.innerHTML = shots.length ? shots.map((shot, index) => `<figure class="capture" data-shot-index="${index}"><img src="${escapeHtml(shot.path)}" alt="SERP screenshot for ${escapeHtml(shot.query)}" loading="lazy" /><figcaption><strong>${escapeHtml(shot.date)}</strong><br />${escapeHtml(shot.query)}</figcaption></figure>`).join("") : '<p class="empty">No SERP screenshots have been captured yet.</p>';
}
function openHistory(url) {
  ...
  root.innerHTML = data ? `<a class="history-url" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(url)}</a><dl class="history-meta"><dt>First seen</dt><dd>${formatDate(data.first_seen)}</dd><dt>Last seen</dt><dd>${formatDate(data.last_seen)}</dd><dt>Disappeared</dt><dd>${formatDate(data.disappeared_at)}</dd><dt>Queries</dt><dd>${escapeHtml((data.queries || []).join(", "))}</dd></dl><canvas id="urlRankChart" height="150"></canvas><h3>Rank history</h3><div class="history-table">${historyRows(data.history || [])}</div><h3>Risk and sentiment changes</h3><div class="history-table">${sentimentRows(data.sentiment_changes || [])}</div>` : `<p class="empty">No exported history for ${escapeHtml(url)}</p>`;
  panel.hidden = false;
  if (data) drawLineChart("urlRankChart", (data.rank_series || []).map((item) => item.date), (data.rank_series || []).map((item) => item.rank ? 11 - item.rank : 0), "Rank visibility");
}
function historyRows(rows) {
  return rows.length ? `<table><tbody>${rows.map((row) => `<tr><td>${formatDate(row.run_datetime)}</td><td>${escapeHtml(row.query)}</td><td>#${escapeHtml(row.current_rank || "-")}</td><td>${badge(row.status)}</td></tr>`).join("")}</tbody></table>` : '<p class="empty">No rows</p>';
}
function sentimentRows(rows) {
  return rows.length ? `<table><tbody>${rows.map((row) => `<tr><td>${escapeHtml(row.date)}</td><td>${badge(row.sentiment)}</td><td>${badge(row.risk_level)}</td></tr>`).join("")}</tbody></table>` : '<p class="empty">No rows</p>';
}
...
function toPoints(size, values, invert) {
  ...
  return values.map((value, index) => ({ x: 36 + (index * (size.width - 60)) / Math.max(1, values.length - 1), y: size.height - 28 - ((invert ? 1 - (Number(value || 0) / maxValue) : Number(value || 0) / maxValue) * (size.height - 56)) }));
}
...
function drawLineChart(id, labels, values, label, invert = false) {
  ...
  drawAxes(ctx, size);
  drawPath(ctx, toPoints(size, values, invert), chartColors[0]);
...
function drawMultiLineChart(id, labels, series) {
  ...
  entries.forEach(([name, values], index) => {
    const points = values.map((value, i) => ({ x: 36 + (i * (size.width - 60)) / Math.max(1, values.length - 1), y: size.height - 28 - ((value / maxValue) * (size.height - 56)) }));
...
function bindEvents() {
  document.querySelectorAll(".view").forEach((button) => button.addEventListener("click", () => { document.querySelectorAll(".view").forEach((item) => item.classList.remove("active")); button.classList.add("active"); document.querySelectorAll(".query-tab").forEach((item) => item.classList.remove("active")); state.view = button.dataset.view; state.queryTab = ""; resetAndRender(); }));
  ["query", "status", "sentiment", "domain"].forEach((key) => document.querySelector(`#${key}Filter`).addEventListener("change", (event) => { state.filters[key] = event.target.value; resetAndRender(); }));
  document.querySelectorAll("[data-sort]").forEach((button) => button.addEventListener("click", () => { const key = button.dataset.sort; if (state.sortKey === key) state.sortDirection = state.sortDirection === "asc" ? "desc" : "asc"; else { state.sortKey = key; state.sortDirection = ["current_rank", "sentiment", "risk_level"].includes(key) ? "asc" : "desc"; } resetAndRender(); }));
  ...
  document.addEventListener("click", (event) => { const historyButton = event.target.closest(".url-history, .text-link"); if (historyButton?.dataset.url) openHistory(historyButton.dataset.url); const capture = event.target.closest(".capture"); if (capture?.dataset.shotIndex) openScreenshot(Number(capture.dataset.shotIndex)); });
  ...
  document.addEventListener("keydown", (event) => { if (document.querySelector("#screenshotModal").hidden) return; if (event.key === "Escape") document.querySelector("#screenshotModal").hidden = true; if (event.key === "ArrowLeft") openScreenshot(state.screenshotIndex - 1); if (event.key === "ArrowRight") openScreenshot(state.screenshotIndex + 1); if (event.key === "+" || event.key === "=") { state.screenshotZoom = Math.min(3, state.screenshotZoom + 0.25); updateScreenshotModal(); } if (event.key === "-") { state.screenshotZoom = Math.max(0.5, state.screenshotZoom - 0.25); updateScreenshotModal(); } });
